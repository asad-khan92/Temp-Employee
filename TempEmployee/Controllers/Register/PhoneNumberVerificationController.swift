...
class PhoneNumberVerificationController: UIViewController {
    ...
    @IBAction func confirmButtonPressed(_ sender: UIButton) {
        ...
        let code:String! = verificationCodeFields.map{$0.text!}.reduce("",+)
...
//
//        let licenceVC : AddLicenseListView = storyboard.instantiateViewController()
//
...
extension PhoneNumberVerificationController{
    ...
    func resendCode(){
        ...
        NumberVerificationService().resendVerificationCode(jobseekerID: Defaults[.employerID]!) { (result) in
            HUD.hide()
            switch result{
    ...
    func setVerificationStatus(){
        ...
        NumberVerificationService().setUserNumberVerified(jobseekerID:Defaults[.employerID]! , completionHandler: { (result) in
            switch result{
